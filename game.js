...
var objects;
var players;
var current_focused_player;
var PLAYER_ACCEL_CONSTANT;
var key_controls;
	...
	var urls = [
		...
		},
		{
			...
			exec: function(data) {
				...
				global_audio_context.decodeAudioData(data, function onSuccess(buffer) {
					for (var obj = 0; obj < objects.length; obj++) {
						if (objects[obj].audio_controller === undefined) objects[obj].audio_controller =
							new Audio();
...
function bind_keys() {
	...
	$(document)
		.keyup(function(e) {
			switch (e.which) {
				case 37: // left
					if (!players[current_focused_player].ai) players[current_focused_player]
						.stop();
...
function createSkyBox() {
	...
	var material = new THREE.ShaderMaterial({
		uniforms: uniforms,
		vertexShader: document.getElementById('sky-vertex')
			.textContent,
		fragmentShader: document.getElementById('sky-fragment')
			.textContent
	});
...
function add_balls_to_scene() {
	for (var obj = 0; obj < objects.length; obj++) scene.add(objects[obj].mesh);
}

function add_players_to_scene() {
	for (var player = 0; player < players.length; player++)
		if (players[player].enabled) scene.add(players[player].mesh);
}

function add_bounding_cube() {
	var bounding_cube = new THREE.BoxHelper(new THREE.Mesh(new THREE.BoxGeometry(
		MAX_BOUND * 2, MAX_BOUND * 2, MAX_BOUND * 2, 2, 2), new THREE.LineBasicMaterial()));
...
function Ball(position, velocity, color) {
	...
	this.audio_controller = undefined;
	...
	this.mesh = new THREE.Mesh(new THREE.SphereGeometry(RADIUS), new THREE.MeshPhongMaterial({
		color: this.color
	}));
	this.reset = false;
...
function Player(args) {
	...
	this.mesh = new THREE.Mesh(new THREE.BoxGeometry(SIZE, SIZE, SIZE / 8), new THREE
		.MeshBasicMaterial({
			color: this.color,
	...
	*/
	...
	this.move_right_ai = function() {
		return function(self) {
			if (self.velocity.x < 0) self.stop_x();
	...
	this.move_up_ai = function() {
		return function(self) {
			if (self.velocity.y < 0) self.stop_y();
		...
		if (this.ai) {
			...
			switch (this.num) {
				case 0:
					...
					if (target_position.z > this.position.x) instructions[1] = this.move_right_ai();
					else if (target_position.z < this.position.x) instructions[1] = this.move_left_ai();
					else if (target_position.z == this.position.x) instructions[1] = this.stop_x_ai();
					if (target_position.y > this.position.y) instructions[2] = this.move_up_ai();
					else if (target_position.y < this.position.y) instructions[2] = this.move_down_ai();
					else if (target_position.y == this.position.y) instructions[2] = this.stop_y_ai();
					break;
				case 1:
					instructions[0] = target_position.x;
					if (target_position.z < -this.position.x) instructions[1] = this.move_right_ai();
				...
				case 2:
					...
					if (target_position.z > this.position.y) instructions[2] = this.move_up_ai();
					else if (target_position.z < this.position.y) instructions[2] = this.move_down_ai();
					else if (target_position.z == this.position.y) instructions[2] = this.stop_y_ai();
					if (target_position.x > this.position.x) instructions[1] = this.move_right_ai();
					else if (target_position.x < this.position.x) instructions[1] = this.move_left_ai();
					else if (target_position.x == this.position.x) instructions[1] = this.stop_x_ai();
					break;
				case 3:
					instructions[0] = target_position.y;
					if (target_position.z < -this.position.y) instructions[2] = this.move_up_ai();
...
function randNorm() {
	return ((Math.random() + Math.random() + Math.random() + Math.random() + Math
		.random() + Math.random()) - 3) / 3;
}

function initialize_variables() {
	SKYBOX_MAX_RADIUS = 20000;
	MAX_BOUND = 2000;
	PRINT_LOGS = false;
	ROTATION_STEP = 0.05;
	objects = [
	...
	players = [
		new Player({
			num: 0,
			color: get_palette_color(0),
			ai: true,
			userControlled: false
		}),
		new Player({
			num: 1,
			color: get_palette_color(1),
			ai: true,
			userControlled: false
		}),
		new Player({
			num: 2,
			color: get_palette_color(2),
			ai: true,
			userControlled: false
		}),
		new Player({
			num: 3,
			color: get_palette_color(3),
			ai: true,
			userControlled: false
		}),
		new Player({
			num: 4,
			color: get_palette_color(4),
			ai: true,
			userControlled: false
		}),
		new Player({
			num: 5,
			color: get_palette_color(5),
			ai: true,
			userControlled: false
		})
	];
	current_focused_player = 0;
	PLAYER_ACCEL_CONSTANT = 8;
	key_controls = {
		left: false,
		right: false,
		up: false,
